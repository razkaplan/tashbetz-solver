...
solver/sweep.py evals/run_eval.py`: zero commits (the intervening DAILY.md entries are
all `nativ`/topic-crossword site-game work, a different part of this repo).

Four searches: (1) "cryptic crossword clue solving candidate generation LLM definition
span 2026"; (2) "retrieval augmented crossword solver BM25 definition retrieval arxiv
2026"; (3) "Hebrew morphology NLP crossword wordplay generation 2026"; (4) "cryptic
crossword definition location start end clue heuristic 2026". Nothing new and buildable:
the same paper set resurfaces (2506.04824, 2406.09043/NAACL 2025, 2407.08824, and
2607.26497 "BM25 Wins at Scale" now with a stable arXiv id, still just reconfirming
`retrieve_defs.py`'s existing BM25 choice). One genuinely new citation, KibutzR (ACL
2026, a Hebrew coreference-resolution benchmark) — checked directly against this
project's open problems and does not transfer: coreference resolution answers "which
mentions refer to the same entity," not definition-span location or wordplay-derived
candidate generation.

Search (4) reconfirms the standard literature framing ("the definition sits at one END
of the clue") this project has cited since its first `defspan.py` attempt — motivation
for wiring `retrieve_defs.end_candidates()` into the live candidate pool. **Correction,
added during the 2026-09-18 PR-backlog consolidation that reads this entry**: that
function was already wired in, under the name `defspan_retrieval_candidates`, by PR #41
(2026-09-02) — this run rebuilt it under a new name (`retrieval_end_candidates`) because
it branched off stale `main` rather than the latest unmerged PR head, and so never saw
#41's code. The MEASUREMENT this run produced (12.5% -> 12.5% on 56 clues across 2
freshly-transcribed puzzles, exact same 7 clues hit either way) is real and is now the
mechanism's third independent confirmation (see DAILY.md); the CODE was not shipped as a
second copy of the same call.

## 2026-09-18

No new literature search run today: the scheduled task's own framing designated PR-
backlog consolidation (queue item 6) as today's one lever, and DAILY.md's own standing
practice treats a consolidation-only day the same way PR #56 did — a legitimate day's
work that does not also require a new mechanism or a fresh literature sweep on top of it.
Per the task's own instruction, did one quick check anyway for anything published very
recently: "cryptic crossword candidate generation LLM 2026" and "Hebrew NLP crossword
wordplay 2026" surfaced nothing beyond 2026-09-17's own four-search pass yesterday (same
