...
def generate(clue_text, enum, pattern=None, max_n=25, use_culture=True, use_retrieval=True,
    ...
    behind the cheap mechanisms either. double_definition_candidates is placed in the same
    tier: it requires an answer to rank in BOTH of two independent BM25 queries (stricter,
    lower-volume than either retrieval_candidates or culture_category alone), so it never
    needs to wait behind the window-scan mechanisms either. `use_culture`/`use_retrieval`/
...
def recall_eval(dataset_path, split=None, max_n=25, use_culture=True, use_retrieval=True,
    ...
    for line in open(dataset_path):
        ...
        cands = generate(r['clue_text'], r['enum'], max_n=max_n, use_culture=use_culture,
                          use_retrieval=use_retrieval, use_double_def=use_double_def)
...
def main():
    ...
    if cmd == 'selftest':
        ...
        rest = [a for a in rest
                if a not in ('--no-culture', '--no-retrieval', '--no-double-def')]
