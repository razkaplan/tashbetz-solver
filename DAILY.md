...
misses decompose into 2-3 already-known dictionary words), far bigger than `--prefix`'s
own 1-genuine-hit finding. Wired into `hidden_candidates`/`reversal_candidates` as a real
(if narrow) generator change; `recall@N` measured UNCHANGED (4.0% both ways) — an honest
negative for THIS specific wiring, not for the underlying diagnostic. See log for the full
transcription/audit trail (including a genuinely difficult grid-orientation puzzle this
run had to work through by triangulating against this project's own historical record).
Did NOT wire phrase_split into anagram/substitution/charade (each needs a structurally
different change, a reverse-search or fragment-source redesign, not attempted today); did
NOT merge or act on any of the 18+ open PRs; did NOT push to `main`.

Previous lever (2026-09-20): **`hebrew-words-db`-as-lexicon, closing the OTHER half of
...
independently-transcribed puzzles' missing gold answers. Did NOT merge or act on any of
the other 18 open PRs; did NOT re-run yesterday's consolidation (see below, already done).

Previous lever (2026-09-18): **queue item 6, "merge or close the PR backlog" — its worst
recurrence yet.** `main` had had no solver-lever merge since 2026-08-30: by today, 18
solver-pipeline PRs (#38 through #61, 2026-08-31 to 2026-09-17) plus one unrelated SEO PR
(#50) were open and unmerged, the largest backlog this queue item has ever recorded
(previous worst was 11, 2026-09-10). Per the scheduled task's own explicit framing, THIS
consolidation is today's one lever — no new candidate-generation mechanism was attempted
on top of it, matching the precedent PR #56 ("no new recall number") already set for a
consolidation-only day being legitimate single-day work.

Read every PR's actual diff and body rather than trusting titles (`pull_request_read
get`/`get_diff` for all 18+1). Found the backlog was NOT flat: PR bodies documented their
own prior reconciliations, verified directly against the branch content rather than taken
on faith (`git show <branch>:solver/candidates.py | grep` for every mechanism function
name expected from #38 through #59 — all present). The chain: #46 folded in #38/#39/#41/
#42/#44 (5-PR backlog); an untracked-in-this-run PR #51 (already closed by the time this
run started — not one of the 18) folded in #46/#47 on top; #54 built on #51 and merged
#52/#53; #56 built on #54 and reconciled a duplicate `container_candidates` implementation
in #55 (kept #54's strictly more general version, added nothing from #55's narrower one);
#57 built on #56's head and added `container_candidates`' entity-fragment source; #58
built on #57's head and added `abbreviation_candidates`; #59 built on #58's head and added
`lexicon_coverage_eval`; #60 built on #59's head and added `prefix_stripped()`. **Every one
of #38 through #60 (all 15 of them present in this run's 18-PR list, plus #51 and #55
which were not) is therefore already fully subsumed by #60's own branch head
(`daily/2026-09-16-work`)** — confirmed structurally, not just by reading claims: `git show
origin/daily/2026-09-16-work:solver/candidates.py` contains every mechanism function named
in every one of those 15 PRs' own titles, and `git diff origin/main...origin/daily/2026-09-16-work
--stat` touches exactly 5 files (`DAILY.md`, `RESEARCH.md`, `solver/candidates.py`,
`solver/deffit.py`, `solver/prove.py`), matching the sum of what those 15 PRs individually
claimed to add.

**#61 (2026-09-17, the most recent PR) broke the chain**: it branched off stale `main`
(pre-#38, the same commit every other PR in the backlog also branched from) instead of
#60's head, the exact compounding-loss pattern this queue item has now flagged six times.
Its own diff adds a new `retrieval_end_candidates()` function to `solver/candidates.py`
that calls `retrieve_defs.end_candidates()` — but reading #60's own already-merged
`defspan_retrieval_candidates()` (added by #41, 2026-09-02, itself already folded into the
chain above) shows it makes the **identical call**, held-out-safe the same way, differing
only in its mechanism label string (`defspan_retrieval` vs `retrieval_end`) and docstring.
#61's own PR body never mentions `defspan_retrieval_candidates` and does not appear to have
read `solver/candidates.py`'s existing contents past `retrieval_candidates()` before adding
a second, functionally-duplicate wrapper around the same call — an unmerged-backlog
consequence one layer down from the branch-off-stale-main problem itself: even reading the
(stale) `main` alone would have missed this, since `defspan_retrieval_candidates` only
exists in the unmerged branch #61 never looked at.

**Decision: did NOT add `retrieval_end_candidates` as new code.** Adding a second function
that makes the same call under a different name would be exactly the kind of drift this
queue item exists to stop, not fix. #61's own MEASURED RESULT is real, new evidence,
though (a fresh, independently-transcribed 2-puzzle/56-clue negative confirmation of the
SAME mechanism, vs. #41's original single-puzzle/28-clue measurement) — folded into the
state table (`defspan_retrieval_candidates` row, above) and the log below, reattributed to
its actual, already-merged mechanism rather than duplicated as if it were new. See the log
entry dated 2026-09-17 for the measurement detail, disclosed as the reframing it is, not
silently absorbed.

**#50 (SEO, unrelated to the solver pipeline) was ATTEMPTED, then deliberately left OUT.**
Its diff touches 9 files (`app/build_defs.py`, `app/build_seo.py`, `app/build_topics.py`,
`app/build_words.py`, `app/patch_milon_kibbutz_split.py`, `app/sitemap_lastmod.py` [new],
`docs/sitemap.xml`, `marketing_kb/TRACKING.md`, `tests/test_sitemap_lastmod.py` [new]) —
zero FILE overlap with the 5 files the solver-side consolidation touches, so it applied
with no textual git conflict. But applying it and running its own new test suite
(`tests/test_sitemap_lastmod.py`) surfaced a real correctness problem, not a merge
conflict: 1/17 tests fails on current `main`'s tip.
`test_sitemap_dates_match_git_for_a_sample` expects every sampled `<lastmod>` in the
committed `docs/sitemap.xml` to match that page's own last-touching git commit date, and
3 of 40 sampled URLs disagree — `/nativ/`, `/solve/`, `/research/`, each dated 2026-09-01
or 2026-09-04 in #50's committed sitemap but actually last touched 2026-09-07 by PR #49
("game-bugs-ux"), which merged to `main` (the current tip) AFTER PR #50 branched and
computed its lastmod dates. This is exactly the failure mode #50's own test exists to
catch, on 3 pages #50's own author never saw change. Fixing it properly means re-running
`app/sitemap_lastmod.py`'s date computation against `main`'s CURRENT git history, which is
SEO/website work, not this run's one lever (solver-pipeline PR-backlog consolidation) —
so rather than ship a known-broken interaction or silently drop the finding, this PR
leaves #50 OUT and reports the exact reproducing failure here. **#50 should NOT be closed
by this PR** — it needs a fresh run of `app/sitemap_lastmod.py` against current `main`
(and its own test suite passing 17/17) before it is safe to merge, independent of this
consolidation.

**What this branch actually contains**: #60's full tree for its 5 files (verbatim, not
re-derived) + this run's own DAILY.md/RESEARCH.md reconciliation (this section, the
reframed 2026-09-17 log entry, and this 2026-09-18 entry) + the state-table update above.
No new candidate-generation mechanism, per today's one-lever budget being spent entirely
on the backlog itself. #50's files are untouched by this branch.

VERIFIED (not re-measured): `./bootstrap.sh --dev-only` run fresh (data/ is gitignored),
then every selftest the consolidated diff touches: `python3 solver/candidates.py selftest`,
`solver/retrieve_defs.py selftest`, `solver/lexicon.py selftest`, `solver/prove.py
selftest`, `solver/substitutions.py selftest`, `solver/deffit.py selftest` (exact results
in this run's own log entry below — this paragraph is the summary the top-of-file section
is for; see the log for the literal command output).

HONEST READ, per this project's own standard: this run did **NOT** re-run any of the 18
PRs' own historical recall/precision/coverage measurements from scratch. Those numbers
already went through this project's mandatory audit gate individually, at the time each
PR was authored (held-out checks, provenance checks, implausibility checks — all disclosed
in each PR's own body, spot-checked by this run's reading of those bodies, not re-executed).
This run's own verification is narrower and disclosed as such: the code merges cleanly,
imports without error, and every listed selftest passes on the consolidated tree. A
reader should not treat "selftests pass" as re-confirming e.g. #38's 0/56 culture-category
result or #60's 32.1% lexicon-coverage number — those stand exactly as their own PRs
measured and audited them, carried forward here unchanged.

STANDING OBSERVATIONS, checked whether they moved, neither did: (1) "DAILY.md itself is a
leak vector" (flagged 2026-08-22, disclosed again every run since) — still unaddressed;
this run did not read answer-bearing prior entries before choosing what to consolidate
(nothing here required looking at gold strings), but the underlying fix (redacting
answers from log prose, or splitting required-reading history from an answer-bearing
appendix) remains a real, undone process question. (2) "Who merges these PRs" — #60's own
log entry already named this explicitly ("nothing is reaching production or being
validated end-to-end by a human"); still true today, more so: this run's own PR is the
19th consecutive open, unmerged solver-lever PR if it is not merged before tomorrow's run.
Per this project's explicit standing practice (repeated in every consolidation PR's own
body, including #25/#46/#54/#56/#57/#58/#59/#60), this run did **not** merge anything
itself — only the project owner merges PRs here, even though this session technically has
merge access.

NOT DONE, honestly: did not attempt any new candidate-generation lever (by design, per the
task's own one-lever framing for today); did not re-verify any prior day's measured
recall/precision/coverage number; did not reorder DAILY.md's Log section into one strictly
linear chronological sequence (it already was not one before today — several consolidation
PRs inserted their own new entries at different points in the file rather than at the
literal end — and re-flowing 5,000+ lines of prior audit trail for pure presentation was
judged not worth the risk of silently altering someone else's measured text mid-edit;
today's own new entries are appended at the file's end, findable by date); did not resolve
whether PR #51 (referenced by #54's own body as already-closed, not one of the 18 named in
this run's task) needs any separate attention — it does not appear in the current open-PR
list, so it is assumed already closed by the project owner.

Previous lever (2026-09-17): **`retrieve_defs.end_candidates()` wired into `candidates.py`
— except, per the finding above, this is a NAME COLLISION with already-merged work, not a
second independent lever.** PR #61 built `retrieval_end_candidates()` unaware that PR #41
(2026-09-02) had already added `defspan_retrieval_candidates()`, calling the identical
`retrieve_defs.end_candidates()` function the identical held-out-safe way. Folded in here
as a RE-MEASUREMENT of that existing mechanism, not new code: re-transcribed TWO full dev
puzzles fresh (2026-05-21 + 2026-05-29, 56 clues, 0 enum mismatches, 0/15 solved-grid row
mismatches each), re-crawled `private_defs` (mordo 50,549 raw/48,933 parsed under a 900s
budget; note.co.il 545 pairs under a 600s budget — both smaller than 2026-08-30's peak
since time-boxed rather than run to plateau, but real, substantial corpora). MEASURED,
controlled before/after (`python3 solver/candidates.py recall data/dataset/clues.jsonl
eval [--no-retrieval-ends]` — note: the underlying code path this flag toggles IS
`defspan_retrieval_candidates`, reachable via its own pre-existing `--no-defspan-retrieval`
flag; #61's flag name is disclosed here as redundant, not added): mechanical-only baseline
**1.8% (1/56)**; **+ culture + whole-clue retrieval: 12.5% (7/56)**; **+ end-anchored
query added: 12.5% (7/56), UNCHANGED** — and isolated (end-anchored alone, whole-clue off):
also 12.5% (7/56), the EXACT SAME 7 clues by direct set comparison (`end_set - whole_set`
and `whole_set - end_set` both empty). AUDITED: `lexicon.held_out_answers()` and
`retrieve_defs.held_out()` both confirmed to block all 56 of both puzzles' own gold
answers; provenance of all 7 hits confirmed `pid=None` (external private_defs corpus) with
a clean semantic fit to their clues (e.g. רוקפור/Roquefort's docs are cheese-type lists,
ברישניקוב's docs are about a Russian ballet dancer); no forbidden reads (14across
hard-walled every attempt, only public CDN images + the two public crawl sources used); no
implausible jump (12.5%→12.5% is exactly 0 points). HONEST READ: a clean, well-powered
(56 clues, 2 puzzles, exact set comparison not just a count) reconfirmation that
end-anchored retrieval finds nothing the whole-clue query does not on this corpus — now
THREE independent measurements of this exact mechanism (2026-09-02's original 1-puzzle/
28-clue result, folded into the chain via #41/#46, plus this 2-puzzle/56-clue result),
all agreeing. Research (RESEARCH.md, full entry there): 4 searches, nothing new and
buildable, consistent with the now 10+ consecutive passes finding this project's standing
literature set (2506.04824, 2406.09043, 2407.08824, 2607.26497) unchanged; one new citation
(KibutzR, Hebrew coreference benchmark, ACL 2026) checked directly and does not transfer
(coreference resolution answers a different question than definition-span location or
candidate generation).

Previous lever (2026-09-16): **`lexicon_coverage_eval` extended with a `--prefix`
...
genuinely fires on real data (2,863 + 645 candidate fragments from 2 trigger-matching
clues) even though none of them completed a real word on this puzzle.**

Started by reading the actual repository state, not just stale `main`: `list_pull_requests`
showed **14 open, unmerged PRs** (#38 through #56, spanning 2026-08-31 to 2026-09-12) —
`main` itself has not had a solver-lever merge since 2026-08-30. Branched this run off PR
#56's head (2026-09-12, `mergeable_state: clean`, already folds in #38/#39/#41/#42/#44/#46/
#47/#52/#53/#54/#55) rather than off stale `main`, per queue item 6's own repeated finding
that branching off `main` instead of the latest unmerged work is exactly what causes the
backlog to compound.

Research (RESEARCH.md, full entry): searched for external resources that resolve a Hebrew
role/category noun ("the judge") to a specific NAMED entity — the gap #55/#56 diagnosed
(שופט/השופט never maps to a specific judge in the mined substitution table). Found nothing
new; this is now the fourth or fifth research pass (2026-08-22/23/24/25, now 09-13) landing
on the identical conclusion that no such external resource exists for Hebrew. Built the
internal capability instead, per queue item 9's own standing guidance to stop re-searching
a closed door and work from this project's own data.

**What changed**: `container_parts()` (solver/candidates.py) gained a third source
alongside the existing literal/destemmed-word and mined-substitution ones — when a clue
word (or its destemmed stem) matches a CATEGORY_TRIGGERS role/category word (song, artist,
politician, kibbutz, etc.), every named entity in that category from culture.json becomes a
candidate inner/outer fragment for the container splice, not just a whole-answer candidate
the way `culture_category_candidates` already uses it. New `entity=True` parameter on
`container_parts()`/`container_candidates()`, threaded through `generate()`/`recall_eval()`
as `use_container_entity` (default True) and a new `--no-container-entity` CLI ablation
flag, so the new source's contribution is independently measurable, not bundled invisibly
into `use_container`. Three new selftest cases prove: (1) an entity-sourced fragment
(from an INJECTED synthetic category table, same discipline as every other mechanism's
selftest) actually completes a real lexicon word via container splicing when the
substitution table is empty and the fragment shares no letters with the clue word that
triggered it; (2) `entity=False` disables ONLY that path, not the pre-existing two; (3) the
mechanism's own docstring updated to disclose the honest caveat that culture.json currently
has no "judge" category at all, so this closes the GENERATOR gap #55/#56 diagnosed, not
necessarily that exact clue's DATA gap.

**Measured**: re-transcribed 2026-05-29 (the project's most independently-verified dev
puzzle, now a 9th time) fresh from `data/images/2026-05-28.jpg` (14across hard-walled again
this run — killed after 10/52 `None: 0 clues` fetches, matching every run since 2026-09-11
— worked entirely from the public-CDN image-fallback technique). Every one of the 28 enum
sums validated against the GRID-DERIVED slot length (`solver/grid_tools.py validate`: OK,
0 mismatches) before any gold letters were touched. GOLD LETTERS came from the small
solved-grid recap in `data/images/2026-06-04.jpg`, calibrated PROGRAMMATICALLY: found the
grid's pixel bounding box and per-cell size via a darkness threshold, cropped and read each
of the 15 rows individually (not one whole-grid crop, which drifts row-by-row at this
resolution — bootstrap.sh's own documented warning), then cross-checked the black-cell
pattern cell-for-cell against the committed `data/grids/2026-05-29.json` via a Python
script (not eyeballed): **0/15 row mismatches** on the first fully-corrected pass (two
transcription slips — a reversed row and a miscounted black-cell position — were caught
BY this exact automated check and fixed before trusting any letter, not after). Three
independent cross-checks against this project's OWN prior citations of this exact puzzle
confirm the transcription is correct: 22A `אנזימימ` (matches 2026-09-05's log), 26A
`פחותאבלכואב` (matches 2026-08-25's log), 1D `ברישניקוב` (matches 2026-08-28's log) — all
three derived independently here via grid calibration, not copied from those entries.
`python3 solver/candidates.py recall data/dataset/clues.jsonl eval [--no-container-entity]`:
mechanical-only baseline 3.6% (1/28); +container without entity 3.6% (1/28, unchanged);
+container WITH entity 3.6% (1/28, unchanged); full defaults 3.6% (1/28, unchanged) — see
the state table's container_candidates row for the full by-mechanism breakdown and the
fragment-count diagnostic proving the new source is live, not silently inert.
...
`gold_norm - blocked` empty for both. The new entity source's own data dependency,
`candidates.culture()`, was checked directly and separately: its FILTERED output has zero
overlap with this puzzle's gold set, and — the more informative check — the RAW, unfiltered
`solver/lex/culture.json` DOES contain 2 of this puzzle's 28 gold answers (`ישפרחימ`/"יש
פרחים" under `song`, `טליגוטליב`/"טלי גוטליב" under `politician`), both correctly excluded
by `culture()`'s existing held-out filter — proof the filter is doing real work on this
exact puzzle, not a no-op that happens to pass because nothing was ever at risk. No
forbidden reads: 14across was never queried for this puzzle's gold data this run, only the
two public CDN images. No jump to explain: 3.6% stayed 3.6%, the opposite of an implausible
result. All 6 affected selftests (`candidates.py`, `retrieve_defs.py`, `lexicon.py`,
`prove.py`, `substitutions.py`, `solve_pass.py` — the last now runnable since a real
`data/dataset/clues.jsonl` exists this run) re-run clean.

HONEST READ: this is a real capability addition with a clean (for this source specifically)
negative single-puzzle result, not a coverage win. The mechanism now generates thousands of
real entity-sourced candidate fragments when a category trigger fires — proven on live
data, not just a synthetic selftest — but on this one puzzle none of them happened to
complete a real dictionary word via container splicing. This does not retire the standing
diagnosis from 2026-09-11/12 (this puzzle's own real container clue needs an entity fact
the generator still cannot reach without a "judge" category in culture.json); it does show
the generator-side gap that diagnosis named is now closed in principle.

NOT DONE, honestly: did not grow culture.json with a "judge" category or any new entities
(a data-curation task, not this run's one lever); did not re-measure on 2026-06-05 (the
puzzle #55/#56 used, which has the actual motivating clue) — 14across's hard wall this run
made real crowd-verified gold data unavailable for it, and re-attempting the image-fallback
solved-grid technique there was the exact approach PR #56 already found unreliable at
letter-OCR resolution without more tooling; did not merge or otherwise act on any of the 14
open PRs (only the project owner can merge); did not act on the still-standing DAILY.md-as-
leak-vector observation (flagged 2026-08-22, disclosed again 2026-08-30) beyond continuing
to disclose cross-checks against this file's own prior citations honestly.

Previous lever (2026-09-12): **found and fixed a duplicate-PR branch-hygiene failure on
...
1. **Candidate generation** — the measured bottleneck. `solver/candidates.py` (2026-08-06)
   ...
   **2026-09-13 BUILT THE THIRD FRAGMENT SOURCE**: `container_parts()` gained
   `entity=True`, reusing CATEGORY_TRIGGERS + culture.json exactly as flagged above.
   MEASURED FLAT (3.6%/1/28, unchanged) on a re-transcribed 2026-05-29 — but PROVED the
   source fires on real data (2,863 + 645 candidate fragments from 2 trigger-matching
   clues, culture.json's own song/author namelists), just didn't complete a real word on
   this puzzle. Closes the GENERATOR-side half of the gap this item names; the DATA-side
   half (culture.json still has no "judge" category, so 2026-06-05's own motivating clue
   is still unreachable in practice) is the honest next step if revisited. See log.
...
10. **[NEW 2026-09-16] Lexicon coverage is a structural ceiling, confirmed on TWO puzzles
    ...
    **2026-09-21 update: attacked sub-item (b) directly -- the multi-word-PHRASE category
    itself.** `phrase_split(w, words)` (candidates.py) recognizes a candidate as `lex()`-
    equivalent when it is the unbroken concatenation of 2 (or 3) words already in `lex()`
    -- no invented rule, no external resource, the same test a human solver applies to an
    unfamiliar-looking string. MEASURED, via the new `lexicon-coverage --phrase` flag on a
    fresh 2026-05-29 transcription (26/28 clues -- two clues honestly excluded, see log):
    of the 16 gold answers still NOT in `lex()` at baseline (36.0%/9-25 coverage), **10
    (62.5%) recover** via a 2-3-way phrase split (`משהרבנו`->`משה`+`רבנו`; `ישפרחימ`->
    `יש`+`פרחימ`; `ברישניקוב`->`בר`+`יש`+`ניקוב`; 7 more) -- by far the largest single
    recovery this queue item has measured, dwarfing (a)'s own 1-genuine-hit-of-3 finding.
    Wired as a real (if narrow) generator-side acceptance path into `hidden_candidates`/
    `reversal_candidates` (`use_phrase=True`/`--no-phrase`): `recall@N` measured UNCHANGED,
    4.0% (1/25) with the toggle either way, because none of the 10 phrase-recoverable
    answers sit as a literal (or reversed) contiguous run of clue letters -- they are
    charade/definition/culture-reference outputs, a different mechanism family entirely.
    Honest reframing, not a reversal: the DIAGNOSTIC confirms this sub-item is real and
...
- 2026-09-13: **`container_candidates` gained the THIRD fragment source 2026-09-12's log
  named as the concrete next step: role/category ENTITY lookup**, reusing
  `culture_category_candidates`'s CATEGORY_TRIGGERS vocabulary and culture.json's named-
  entity lists as a candidate inner/outer fragment source (alongside the existing literal/
  destemmed-word and mined-substitution sources), so a container clue whose piece is an
  entity FACT (a specific singer, politician, kibbutz) rather than a literal clue word or a
  generic synonym can now be reached in principle.

  FIRST, read the actual repository state rather than trusting stale `main`:
  `list_pull_requests` showed **14 open, unmerged PRs** (#38-#56, 2026-08-31 to 2026-09-12),
  and `main` has had no solver-lever merge since 2026-08-30. Branched off PR #56's head
  (2026-09-12, already folds in #38/#39/#41/#42/#44/#46/#47/#52/#53/#54/#55,
  `mergeable_state: clean`) instead of off `main`, per queue item 6's own repeatedly-flagged
  finding that branching off `main` instead of the latest unmerged work is what causes this
  backlog to compound in the first place.

  RESEARCH: searched for an external resource resolving a Hebrew role/category noun ("the
  judge") to a specific named entity — the exact gap #55/#56 diagnosed. Found nothing new;
  this is the fourth or fifth research pass (2026-08-22/23/24/25, now 09-13) landing on the
  identical conclusion. See RESEARCH.md for the full search log. Built the internal
  capability instead, per queue item 9's standing guidance to stop re-searching a closed
  door once a research question has returned the same null result this many times.

  IMPLEMENTED: `container_parts()`/`container_candidates()` (solver/candidates.py) gained
  an `entity` parameter (default True) — when a clue word's destemmed stem matches a
  CATEGORY_TRIGGERS word, every entity in that category from `culture()` (already
  held-out-filtered, same discipline as `culture_category_candidates`) becomes a candidate
  fragment. Threaded through `generate()`/`recall_eval()` as `use_container_entity` and a
  new `--no-container-entity` CLI flag, so the new source is independently ablatable, not
  silently bundled into the existing `use_container` toggle. Three new selftest cases:
  an injected entity fragment completing a real lexicon word via container splicing with
  the substitution table empty; `entity=False` disabling only that path; and the module
  docstring disclosing that culture.json currently has no "judge" category, so this closes
  the GENERATOR gap, not necessarily #55/#56's own exact clue's DATA gap.

  TRANSCRIBED 2026-05-29 fresh (9th independent transcription of this project's canonical
  dev puzzle) from `data/images/2026-05-28.jpg` — 14across hard-walled again this run
  (killed after 10/52 `None: 0 clues`, matching every run since 2026-09-11), worked
  entirely from the public-CDN image-fallback technique. All 28 enum sums validated against
  the GRID-DERIVED slot length (`grid_tools.py validate`: OK, 0 mismatches) before any gold
  data was touched. GOLD LETTERS came from the solved-grid recap in
  `data/images/2026-06-04.jpg`, calibrated PROGRAMMATICALLY: computed the grid's pixel
  bounding box and per-cell size from a darkness threshold, cropped and read each of the 15
  rows INDIVIDUALLY (a single whole-grid crop drifts row-by-row at this resolution, per
  bootstrap.sh's own warning), then cross-checked the black-cell pattern against the
  committed `data/grids/2026-05-29.json` with a script, not eyeballing: **0/15 row
  mismatches** — two real transcription slips (a fully reversed row, a miscounted black-
  cell position) were caught by this exact check and fixed before any letter was trusted,
  not after. Three independent cross-checks against this project's OWN prior citations of
  this exact puzzle, all derived here fresh via grid calibration rather than copied,
  confirm correctness: 22A `אנזימימ` (matches 2026-09-05's log entry), 26A `פחותאבלכואב`
  (matches 2026-08-25's), 1D `ברישניקוב` (matches 2026-08-28's).

  MEASURED, controlled (`python3 solver/candidates.py recall data/dataset/clues.jsonl eval
  [--no-container-entity]`): mechanical-only baseline **3.6% (1/28)**; +container without
  entity **3.6% (1/28), unchanged**; +container WITH entity **3.6% (1/28), unchanged**;
  full defaults **3.6% (1/28), unchanged**. Not a silent zero, though: container fired on
  1/28 clues (9 across, 2 candidates, both pre-existing literal/destemmed hits, 0 from
  entity). Checked WHY the entity source didn't move the number, not just that it didn't:
  2 clues in this puzzle carry a category trigger (1 across "שירה"->song, 16 down
  "סופר"->author), and `container_parts(..., entity=True)` generates **2,863 and 645
  candidate fragments respectively** from culture.json's real song/author namelists at
  those two clues — a genuinely large, real fragment pool, not a silent no-op — but none of
  them combined with the OTHER clue word's fragment into a real lexicon word via container
  splicing on this specific puzzle. Wall-clock cost negligible (17.57s -> 17.60s for the
  full 28-clue eval) despite the large fragment counts, since the length-based filter in
  `container_candidates`'s pairing loop keeps it cheap.

  AUDITED (mandatory gate). `lexicon.held_out_answers()` and `retrieve_defs.held_out()`
  both confirmed to block all 28 of this puzzle's own gold answers (`gold_norm - blocked`
  empty for both). The new source's own dependency, `candidates.culture()`, checked
  separately and directly: its FILTERED output has zero overlap with this puzzle's gold
  set — and, more informatively, the RAW unfiltered `solver/lex/culture.json` DOES contain
  2 of this puzzle's 28 gold answers (`ישפרחימ`/"יש פרחים" under `song`, `טליגוטליב`/"טלי
  גוטליב" under `politician`), both correctly excluded by `culture()`'s existing held-out
  filter — proof the filter does real work on this exact puzzle, not a no-op that happens
  to pass because nothing was ever actually at risk. No forbidden reads: 14across was never
  queried for this puzzle's gold data this run, only the two public CDN images. No jump to
  explain: 3.6% stayed 3.6%, the opposite of an implausibility concern. All 6 affected
  selftests (`candidates.py`, `retrieve_defs.py`, `lexicon.py`, `prove.py`,
  `substitutions.py`, and — runnable for the first time in several of these entries since a
  real `data/dataset/clues.jsonl` exists this run — `solve_pass.py`) re-run clean.

  HONEST READ: a real capability addition, cleanly (for this source) measured negative on
  one puzzle — not a coverage win, but not a confounded non-test either, which is what the
  two prior container_candidates measurements (2026-09-03, 2026-09-11/12) actually were.
  The mechanism now provably generates real entity-sourced fragments from live culture.json
  data when a trigger fires; this specific puzzle's specific clues just didn't have a
  matching pair that spliced into a real word. This does not retire 2026-09-11/12's own
  standing diagnosis (2026-06-05's real container clue needs a "judge" entity culture.json
  doesn't have a category for at all) — it closes the GENERATOR-side half of that gap,
  leaving the DATA-side half (growing culture.json's category coverage) as the honest next
  step if this is revisited.

  NOT DONE, honestly: did not add a "judge" category or any new entities to culture.json (a
  data-curation task, not this run's one lever); did not re-measure on 2026-06-05 itself
  (the puzzle with the actual motivating clue) — 14across's hard wall made real crowd-
  verified gold data unavailable, and PR #56 already found the image-fallback solved-grid
  technique unreliable at letter-OCR resolution for that specific puzzle without more
...
- 2026-09-17: **candidate generation (lever 1) — wired `retrieve_defs.end_candidates`
  into `candidates.py` as `retrieval_end_candidates`, believing it closed a real gap
  (the function has existed since 2026-08-08 but nothing in `generate()`'s pool called
  it directly).** RETROACTIVE CORRECTION, made during the 2026-09-18 consolidation that
  reads this entry: this branched off stale `main` (pre-#38) rather than the latest
  unmerged PR head, and `main` at that point had never seen PR #41 (2026-09-02), which
  had ALREADY added `defspan_retrieval_candidates()` calling this exact same
  `retrieve_defs.end_candidates()` function the identical held-out-safe way — a real
  instance of the exact compounding-loss pattern this project's queue item 6 warns
  about, one function deep rather than one whole file deep this time. The measurement
  below is real and is preserved; the CODE this entry describes was not carried forward
  as new (see 2026-09-18's entry) because it would have shipped a second wrapper around
  a call `candidates.py` already made under a different mechanism name.

  BOOTSTRAP: `./bootstrap.sh --dev-only` hit the routine 14across hard wall (4/52
  fetches in a row `None: 0 clues` after full retry-with-backoff) — killed after ~12
  minutes, worked entirely from the no-14across image-fallback technique. Public CDN
  image downloads and the private_defs crawl sources (note.co.il, pitaronfree) were all
  reachable fine, confirming (again) the wall is specific to 14across.co.il's bot-check.
  `crawl_defs.py mordo` (900s budget, time-boxed, not run to completion): 50,549 raw /
  48,933 parsed pairs. `crawl_defs.py note` (600s budget): 545 pairs.

  TRANSCRIPTION: two full dev puzzles, both fresh. **2026-05-29** (clues from
  `data/images/2026-05-28.jpg`, solution from `data/images/2026-06-04.jpg`): 28/28
  clues, 0/28 enum mismatches, 0/15 solution-grid row mismatches against the committed
  grid (gridlines located programmatically via a darkness-threshold row scan, catching a
  one-row drift a naive uniform-spacing crop would have produced). One gold answer,
  `ברישניקוב` (Baryshnikov), independently reproduces this file's own 2026-08-30 finding
  on this exact puzzle from a from-scratch transcription and a different corpus crawl.
  **2026-05-21** (clues from `data/images/2026-05-20.jpg`, solution from
  `data/images/2026-05-28.jpg` — the same image serving as puzzle 2026-05-29's own clue
  source in a different role, both legitimate): 28/28 clues, 0/28 enum mismatches, 0/15
  row mismatches, after catching and fixing a mirrored-reading-direction slip on row 14
  via an individual-cell-crop montage. Two derived answers (`שפטהשופטימ` split (7,3),
  `נילמתומדקא` split (4,6)) did not resolve to a recognized phrase — disclosed rather
  than silently assumed wrong; the grid-pattern match (this project's strongest check)
  passes cleanly on both, so the likely explanation is an idiom/proper noun outside this
  run's vocabulary, not a transcription error, but this is not proven either way.

  MEASURED, controlled before/after, combined 56-clue set: mechanical-only baseline
  **1.8% (1/56)**; **+ culture + whole-clue retrieval: 12.5% (7/56)**; **+ the
  end-anchored query (i.e. `defspan_retrieval_candidates`, under this entry's original
  `retrieval_end_candidates` name): 12.5% (7/56), UNCHANGED**. Isolated (end-anchored
  alone, whole-clue off, culture on): also 12.5% (7/56). Per-clue set comparison, not
  just the count: the whole-clue-retrieval hit set and the end-anchored hit set are
  EXACTLY the same 7 clues (`[('2026-05-21',12,'across'),('2026-05-21',14,'across'),
  ('2026-05-21',4,'down'),('2026-05-29',23,'across'),('2026-05-29',26,'across'),
  ('2026-05-29',1,'down'),('2026-05-29',21,'down')]`) — `end_set - whole_set` and
  `whole_set - end_set` both empty.

  AUDITED (mandatory gate). `lexicon.held_out_answers()` and `retrieve_defs.held_out()`
  both confirmed to block all 56 of both puzzles' own gold answers. Provenance of all 7
  retrieval hits checked directly: every matching document carries `pid=None` (external
  private_defs corpus), and the underlying definition text is a clean semantic fit (e.g.
  `רוקפור`/Roquefort's docs are cheese-type lists, matching clue 14's "dedication to
  cheese"; `ברישניקוב`'s docs are about a Russian ballet dancer). No forbidden reads:
  14across was never successfully queried this run. Implausibility check: 12.5%→12.5% is
  exactly 0 points, nowhere near the ~15-point suspicion bar.

  HONEST READ: a clean, well-powered (56 clues, 2 independent puzzles, exact set
  comparison) NEGATIVE result for the end-anchored query as an INCREMENTAL source over
  whole-clue retrieval — now the mechanism's THIRD independent confirmation (2026-09-02's
  original 28-clue measurement via #41, now this 56-clue one), all agreeing. The most
  likely explanation, consistent with `retrieval_candidates`'s own docstring: this
  setter's clues are short (mean ~6 letters) and this project's private_defs definitions
  are themselves short phrases, so whole-clue and end-anchored BM25 queries converge on
  the same small matching-document set most of the time.

  RESEARCH (full entries in RESEARCH.md): four searches (candidate generation/
  definition-span LLM approaches; BM25/retrieval-augmented crossword solving; Hebrew
  morphology + crossword wordplay; definition-location heuristics). Nothing new and
  buildable — the same paper set resurfaces (2506.04824, 2406.09043, 2407.08824, and
  2607.26497 now with a stable arXiv id) plus one new-since-2026-08-30 citation
  (KibutzR, a Hebrew coreference-resolution benchmark, ACL 2026) that does not transfer:
  coreference resolution answers a different question than definition-span location or
  wordplay-derived candidate generation.

  NOT DONE, honestly (as originally written, still accurate): did not transcribe the
  third chainable puzzle (2026-06-05) to keep this run to two puzzles; did not resolve
  the two semantically-unresolved answers beyond the row-level re-verification already
  described; did not act on the standing DAILY.md-as-leak-vector finding beyond
  continuing not to read answer-bearing prior entries before choosing which puzzle to
  transcribe; did not merge or otherwise act on any open PR. **NOT KNOWN AT THE TIME,
  disclosed now**: did not check whether this exact mechanism already existed under a
  different name in an unmerged sibling branch — see 2026-09-18's entry for the
  correction and its own root-cause read of why branching off stale `main` let this
  happen.

- 2026-09-18: **queue item 6, "merge or close the PR backlog" — consolidated an 18-PR
  solver-pipeline backlog (#38 through #61, 2026-08-31 to 2026-09-17); left a 19th,
  unrelated SEO PR (#50) deliberately OUT after finding its own test suite fails against
  current `main`, the worst backlog this queue item has recorded (previous worst: 11,
  2026-09-10).** Full narrative, the supersession chain, the #61 naming-collision finding,
  the #50 exclusion, and the honest-read/not-done disclosures are in the "Last lever
  added" section at the top of this file — this entry is the chronological log record,
  not a duplicate of that reasoning.

  Read all 19 PRs' actual diffs and bodies via `pull_request_read` (`get`/`get_diff`),
  not titles. Verified structurally, not just by reading claims, that PR #60
  (`daily/2026-09-16-work`) already contains every mechanism function named across
  #38/#39/#41/#42/#44/#46/#47/#52/#53/#54/#55/#56/#57/#58/#59 (`git show
  origin/daily/2026-09-16-work:solver/candidates.py | grep` for each function name; all
  present) and that its diff against `main` touches exactly the 5 files those PRs
  collectively claimed (`DAILY.md`, `RESEARCH.md`, `solver/candidates.py`,
  `solver/deffit.py`, `solver/prove.py`). Built this branch from `main`'s current tip
  (unchanged since all these PRs branched, confirmed via `git log -1 origin/main`) by
  taking PR #60's tree for those 5 files verbatim, then hand-reconciling DAILY.md/
  RESEARCH.md for PR #61's genuinely-new content: PR #61 (2026-09-17) turned out to
  duplicate already-merged work under a new name (`retrieval_end_candidates` vs. the
  already-merged `defspan_retrieval_candidates`, PR #41) because it branched off stale
  `main` instead of #60's head — its CODE was not carried forward for that reason (see
  above), but its MEASUREMENT (a fresh, independent 2-puzzle/56-clue reconfirmation of the
  same mechanism) is real new evidence and was folded into the state table and reframed
  into the log (see the corrected 2026-09-17 entry directly above this one). PR #50's tree
  (9 files) was ALSO applied and tested before being reverted out of this branch: its own
  `tests/test_sitemap_lastmod.py` fails 1/17 against current `main` (3 pages' `<lastmod>`
  dates disagree with git history, because `main` gained a further commit — PR #49 — to
  those pages after #50 branched and computed its dates) — a real correctness problem, not
  a git conflict, and not this run's one lever to fix. Reverted `app/build_defs.py`,
  `app/build_seo.py`, `app/build_topics.py`, `app/build_words.py`,
  `app/patch_milon_kibbutz_split.py`, `docs/sitemap.xml`, `marketing_kb/TRACKING.md` to
  `main`'s versions and removed the two new files (`app/sitemap_lastmod.py`,
  `tests/test_sitemap_lastmod.py`) rather than ship a known-broken interaction; confirmed
  `app/url_guard.py` still passes clean (6,071 URLs, none dropped) on the reverted tree.

  VERIFIED on the consolidated branch: `./bootstrap.sh --dev-only` run fresh (data/ is
  gitignored, every run starts from zero), then every selftest the consolidated diff
  touches: `python3 solver/candidates.py selftest`, `python3 solver/retrieve_defs.py
  selftest`, `python3 solver/lexicon.py selftest`, `python3 solver/prove.py selftest`,
  `python3 solver/substitutions.py selftest`, `python3 solver/deffit.py selftest` — all
  ALL PASSED / exit 0 (see this run's shell transcript; not re-pasted here to avoid
  duplicating tool output DAILY.md does not otherwise carry verbatim).

  HONEST READ: this run did NOT re-run any of the 18 PRs' own historical recall/
  precision/coverage/yield measurements from scratch — those numbers already went
  through this project's mandatory audit gate individually, at the time each PR was
  authored, and are carried forward here exactly as measured, not re-derived. This run's
  own verification is narrower: the consolidated code imports and runs, and every listed
  selftest passes. A reader should treat every specific recall/precision number in this
  file's history as standing on its OWN originating PR's audit, not on today's
  consolidation.

  STANDING OBSERVATIONS, re-checked, neither moved: "DAILY.md itself is a leak vector"
  (flagged 2026-08-22) — still unaddressed, a real undone process question, not touched
  today beyond continuing the discipline of not reading answer-bearing entries before
  choosing what to consolidate (nothing here required it). "Who merges these PRs" — #60's
  own 2026-09-16 entry already named this explicitly; still true, more so: this run's own
  PR becomes the single open solver-lever PR against `main` if merged promptly, or the
  19th consecutive day of a growing backlog if not. This run did NOT merge anything
  itself and did NOT push to `main` — per this project's own repeated, explicit standing
  practice, only the project owner merges PRs here.

  NOT DONE, honestly: no new candidate-generation lever was attempted (by design — this
  IS today's one lever, per precedent PR #56 already set); no prior measurement was
  re-verified beyond the selftests listed above; DAILY.md's Log section was not
  re-flowed into one strictly linear chronological sequence (several prior consolidation
  PRs had already inserted entries at different points in the file rather than at its
  literal end, and re-ordering 5,000+ lines of prior audit trail risked altering someone
  else's measured text for a purely cosmetic gain — today's own two new entries are
  appended here, at the file's true end, and cross-referenced by date from the top-of-
  file section instead); did not resolve whether PR #51 (referenced only by #54's own
  body, already closed before this run started, not one of the 18 named in today's task)
  needs separate owner attention — it is not in the current open-PR list, so it is
  assumed already closed.

- 2026-09-19: **`private_defs` wired into `lexicon.py` as a lexicon-MEMBERSHIP source,
  closing 2026-09-16's own explicitly-disclosed gap** ("NOT wired into `is_word()`/
  candidate generation" — that run's `prefix_stripped()` diagnostic was deliberately left
  unshipped because short stripped stems are coincidence-prone). Branched off yesterday's
  consolidation branch (`daily/2026-09-18-consolidate-18pr-backlog`, PR #62), not stale
  `main`, specifically to avoid the exact branch-hygiene mistake #61 made and #62 itself
  documented (re-deriving work already sitting in an unmerged PR head).

  WHY THIS LEVER: `lexicon_coverage_eval` (2026-09-15/16) measured that only 28.6%/32.1%
  of two dev puzzles' gold answers are members of `lex()` at all — a ceiling every
  mechanical generator in `candidates.py` inherits, since none of them can ever propose a
  string that isn't already a lexicon member, independent of how correct its fodder is.
  This is the single most fundamental unaddressed finding on the board: 18 solver PRs
  since 2026-08-31 added new fragment sources and query shapes on TOP of `lex()`, but none
  touched what `lex()` itself contains. `data/answers/private_defs/` (note.co.il +
  pitaronfree/מורדו, crawled since 2026-08-08) has been used exclusively as
  `retrieve_defs.py`'s BM25 retrieval documents — never as a straight lexicon source, even
  though its answer side is, by construction, a large list of independently-attested real
  Hebrew crossword answers (words, names, phrases) — exactly what `is_word()`-style
  membership checks need, and unlike prefix-stripping it needs no invented morphological
  rule that could manufacture a false positive.

  BOOTSTRAP: `./bootstrap.sh --dev-only` hit the same 14across hard wall as every run
  since 2026-08-19 (2/2 attempted returned `None: 0 clues`; killed after confirming the
  pattern rather than waiting out the full 52, matching established practice) — worked
  entirely from the public-CDN image-fallback technique. `hspell.txt` (129,574 words),
  `culture.json`, and `substitutions.json` are all committed, not gitignored, so those
  loaded from git as usual; only the 14across-derived `answers_parsed.json` was absent
  this run (no committed corpus tier contribution beyond what's already in git — not a
  gap this lever's measurement depends on).

  CORPUS: `scraper/crawl_defs.py mordo` run fresh (gitignored, rebuilt every run) to
  36,148 raw / 34,902 parsed pairs (stopped deliberately at a size in the same range as
  several prior runs, not to a natural plateau — the blogspot feed is effectively
  unbounded, per every prior run's own observation); `crawl_defs.py note` run to full
  completion this time, 1,301/1,301 discovered URLs fetched, 1,297 with parsed answers.
  Combined: 37,449 definition→answer pairs, adding **49,158 words to `lex()`**
  (144,019 → 193,177, confirmed by direct measurement, not assumed from row counts).

  TRANSCRIPTION: two independent dev puzzles, both via the image-fallback technique
  (`data/dataset/`, `data/answers/by_date/` are gitignored and empty in a fresh checkout,
  so neither pre-existed this run despite both puzzles having been transcribed many times
  by prior runs — nothing carries over).
  - **2026-05-15**: clue text from `data/images/2026-05-14.jpg` (9 across clues legibly
    printed — 1, 7, 8, 9, 10, 11, 13, 15, 17 — before the page's own text genuinely cuts
    off mid-clue-19, a real, disclosed content limit of this specific article image, not
    a transcription shortcut). Gold answers for **all 28 slots** (not just the 9 with
    clue text) came from the solved-grid recap in `data/images/2026-05-20.jpg`
    ("פתרון תשבץ ההיגיון מהשבוע שעבר"), grid-calibrated PROGRAMMATICALLY: automated
    darkness-threshold border/cell detection (numpy, not eyeballed crops) located the
    11x15 grid and sampled each cell's mean pixel value; **0/165 cell mismatches**
    against the committed `data/grids/2026-05-15.json`. All 9 enum sums validated against
    the grid-derived slot length from `solver/grid_tools.slots()`: 0/9 mismatches.
  - **2026-05-29**: clue text from `data/images/2026-05-28.jpg` (6 usable across clues —
    1, 7, 8, 9, 10, 11; clue 13 visible but cut off before its enum, excluded rather than
    guessed). Gold answers for all 28 slots from the recap in `data/images/2026-06-04.jpg`,
    same programmatic grid-calibration, 0/165 cell mismatches against
    `data/grids/2026-05-29.json` (confirmed byte-identical in black-cell pattern to
    2026-05-15's grid — this setter's documented fixed-template reuse — so the pixel-match
    alone does not uniquely identify the puzzle; see below for the stronger check that
    does). All 6 enum sums validated: 0/6 mismatches.

  A REAL TRANSCRIPTION ERROR CAUGHT AND FIXED BEFORE ANY MEASUREMENT, disclosed rather
  than silently corrected: an early pass at 2026-05-29's recap grid read 3 of its 15 rows
  backwards (screen right-to-left instead of left-to-right — an easy slip when the eye
  wants to read Hebrew in its natural direction even while mechanically scanning image
  pixels). Caught two ways: (1) `grid_tools.slots()`'s black-cell mismatch check flagged
  one row outright (a non-palindromic pattern that a flip breaks); (2) for the OTHER two
  flipped rows, the black pattern happened to be symmetric enough that the mismatch check
  alone couldn't catch it, so this was caught instead by cross-checking two answers this
  exact puzzle ALREADY has independently documented in this project's own history — `1
  down` = `ברישניקוב` (Baryshnikov, RESULTS.md-adjacent, named in the 2026-08-28 log entry)
  and `26 across` = `פחותאבלכואב` (the "פחות אבל כואב" song, named in the 2026-08-25 log
  entry) — both of which the first-pass reading produced BACKWARDS
  (`סרישניקוב`/`באוכלבאתוחפ`). Re-verified all 15 rows a second time, per-row, by cropping
  and reading only each row's leftmost and rightmost cell in isolation (removing the
  direction ambiguity a whole-row read can introduce); the corrected transcription
  reproduces both pre-existing gold answers exactly, with 0/165 cell mismatches
  maintained throughout. Disclosing this in full because it is exactly the kind of
  silent-direction-flip risk this project's image-fallback technique has always carried
  but not previously had to catch mid-transcription — future image-fallback runs should
  budget for an edge-cell recheck, not just a black-pattern check, especially on rows
  whose pattern is left-right symmetric.

  MEASURED, controlled before/after: **`lexicon_coverage_eval` combined across both
  puzzles (56 gold slots): 32.1% (18/56) with `--no-defs-lexicon` → 32.1% (18/56) with
  `private_defs` ON — UNCHANGED.** `recall data/dataset/clues.jsonl dev` (full pipeline,
  all mechanisms, on the 15 clues with real clue text) also 0.0% (0/15) both ways — but
  this second number is UNINFORMATIVE for this lever specifically, disclosed rather than
  presented as a second confirming result: the partial clue-text sample (necessarily
  missing clue 26 across, the one clue this exact puzzle's own retrieval hit has
  historically landed on per the 2026-08-25 log entry) has a floor-effect 0% baseline with
  every mechanism including `retrieval_candidates`, so it cannot show a private_defs-
  specific effect either way. `lexicon_coverage_eval` (which needs no clue text and covers
  the FULL 56-slot gold set) is the clean, decisive test here.

  AUDITED (mandatory gate), each finding checked directly rather than assumed:
  - **The negative result is not a silent no-op**: directly diffed `lex()` with the
    toggle on/off — 49,158 words genuinely added, confirmed by size (144,019 → 193,177).
  - **The 0/38 overlap is exact, not approximate**: computed the missing-answer set under
    the baseline lexicon and the added-word set under private_defs, and intersected them
    directly — 0 elements, printed and inspected, not inferred from an unchanged
    percentage alone (a percentage could theoretically hide a swap of equal size; a direct
    set intersection cannot).
  - **A real potential leak shape was checked and found NOT to apply**: 28 of the 56 gold
    answers ARE present somewhere in the RAW (unfiltered) private_defs corpus, and 13 of
    those remain in the held-out-FILTERED lexicon — at first glance concerning, but
    checked further: every one of those 13 is ALSO a member of the pre-existing BASE
    lexicon (hspell/corpus/culture) with `private_defs` fully OFF, i.e. an ordinary
    dictionary word that happens to be a gold answer (`ערב`, `נשי`, `שכם`, `שלג`...) — the
    exact category RESULTS.md's own INTEGRITY FINDING already ruled legitimate ("Ordinary
    dictionary words that happen to be answers... legitimately remain, as they would in
    any real solver's dictionary"). Computed the set difference `(gold ∩ private_defs-
    lexicon) − base-lexicon` directly: **empty** — zero gold answers are newly introduced
    specifically BY private_defs. `lexicon.held_out_answers()` and `retrieve_defs.held_out()`
    both confirmed (computed) to block all 56 gold answers across both puzzles.
  - **No forbidden reads**: 14across was attempted only via bootstrap's own sanctioned
    corpus-acquisition step (killed after confirming the hard-wall pattern, never queried
    for either puzzle's specific gold data); gold answers came from the two public CDN
    solved-grid recap images; no answer/solution site was consulted for either puzzle;
    clue text was never searched verbatim online.
  - **Implausibility check**: 32.1% → 32.1% is 0.0 points of movement — the polar opposite
    of an implausible jump; nothing to explain away.
  - All 6 affected selftests re-run clean (exit 0): `candidates.py`, `lexicon.py`,
    `retrieve_defs.py`, `prove.py`, `substitutions.py`, `deffit.py`. One PRE-EXISTING
    selftest (`candidates.py`'s own `charade_candidates` fixture check) started failing
    once private_defs was the default — not a bug in that test's logic, but a real
    side-effect of this lever growing the lexicon by 34%: `charade_candidates`' own
    `max_parts_out=200` combination cap can now be reached by unrelated new hits before
    the fixture's expected combination is tried, since candidate order depends on `lex()`
    dict-iteration order, which private_defs changes. Fixed by pinning
    `set_use_defs_lexicon(False)` at the top of `selftest()` (every pre-existing synthetic
    fixture was tuned against the base lexicon and should stay hermetic to whatever this
    run's gitignored corpus happens to contain); the two NEW checks this lever adds
    explicitly re-enable it where they need to test private_defs itself. Disclosing this
    as a real, if secondary, finding: a large exogenous lexicon addition can shift OTHER
    mechanisms' capped output in production too, not just in this test — worth watching,
    not something this run's one-lever budget re-measured across all 11 other mechanisms.

  HONEST READ: a clean, decisive, negative result on the two puzzles this run could
  transcribe. The private_defs corpus (mostly note.co.il/מורדו definitions for OTHER,
  likely easier/more standard newspaper crosswords) grew the lexicon substantially in
  absolute terms, but not one of the ~37k answers it contributed happens to be among
  either hard-setter puzzle's own missing vocabulary — consistent with a real, plausible
  root cause rather than a bug: יורם הרועה's answers lean on multi-word idioms and
  wordplay-assembled phrases (`מחזיק אצבעות`, `פחות אבל כואב`, `משה רבנו`) that a
  general crossword-help site, aimed at more standard puzzles, is less likely to have
  independently posted as a clean single "definition → answer" pair. This does not mean
  the private_defs-as-lexicon IDEA is dead — `retrieval_candidates`' own history (flat on
  2026-07-10 across three corpus sizes, then strongly positive on 2026-06-26 once BOTH
  the corpus grew AND a full transcription closed a clue-text gap) shows this exact
  corpus is real and puzzle-dependent when used as retrieval documents; this run is the
  first test of the SAME corpus in a structurally different role (lexicon membership
  rather than ranked retrieval), on 2 puzzles, and the honest reading is "not yet shown to
  help, not shown to be broken" — a third puzzle, or a much larger crawl, are the two
  concrete next steps if this is revisited, not a verdict that it can't work.

  NOT DONE, honestly: did not re-measure on a third puzzle (today's one-lever budget went
  to two independent transcriptions plus the mid-run direction-flip investigation, which
  took longer than expected); did not grow the private_defs crawl to its own natural
  plateau (mordo stopped at a size comparable to several prior runs, not exhausted — per
  every prior run's own observation the blogspot feed shows no sign of a fixed end); did
  not attempt to re-measure the FULL clue-text `recall_eval` on a puzzle whose available
  clue text actually includes a clue with a known historical retrieval hit (would need a
  third, differently-cropped image); did not act on the still-standing "DAILY.md is a
...
- 2026-09-21: **candidate generation, queue item 10(b) — phrase-aware lexicon
  membership**, picked up directly from 2026-09-20's own closing note ("10(b) is now,
  on this run's own evidence, the more promising of the two remaining sub-steps").
  Branched off `daily/2026-09-20-hwdb-lexicon` (PR #64's head), not stale `main` —
  main has had no solver-lever merge since 2026-08-30 and PRs #38-#64 (31 PRs) remain
  open and unmerged; #64 is a strict superset of that whole chain per its own PR body.

  BOOTSTRAP: `./bootstrap.sh --dev-only` hit the same 14across hard wall this project's
  log has now documented on more than a dozen runs since 2026-08-19 (14/52 fetches in a
  row came back `None: 0 clues` after full retry-with-backoff each) — killed after
  confirming the pattern, worked entirely from the no-14across image-fallback technique.
  hspell (129,574 words) and hwdb (67,008 words) both fetched cleanly; `solver/lex/
  culture.json` and `solver/lex/substitutions.json` are committed (not gitignored),
  needed no rebuild. Did not crawl `private_defs` this run (out of scope for this
  lever, and `lexicon.py`'s own `include_private_defs` default already exercises
  whatever a future run crawls).

  RESEARCH (full entries in RESEARCH.md): searched for 2025-2026 work on cryptic
  candidate generation, definition-span detection, and Hebrew multi-word-expression/
  compound handling specifically (this lever's own topic). Nothing new on the
  candidate-generation side (same paper set logged repeatedly since 2026-08-06,
  including 2506.04824's own lexicon-membership-filtering stage, already the
  standing architectural precedent this project's `candidates.py`/`prove.py` follows).
  One genuinely relevant check: `github.com/Dvd848/Crossword-Solver`, a Hebrew
  pattern-matching crossword utility (Wiktionary/Wikipedia/Hebrew WordNet/hspell-backed)
  — fetched and read directly, not judged by its description. It is architecturally
  identical to this project's own `lexicon.py pattern` (a single-word `?`-pattern
  matcher over a DAWG), with no multi-word/phrase capability and no new wordlist this
  project doesn't already have access to. **Transfer: a useful negative** — an
  independently-built Hebrew crossword tool landing on the same word-level (not
  phrase-level) architecture corroborates rather than contradicts this queue item's
  own standing finding (2026-08-23 onward) that no external phrase-list resource
  exists for this gap; it has to be built from the project's own lexicon structurally,
  which is exactly what this run did.

  TRANSCRIPTION: re-transcribed the canonical dev puzzle (2026-05-29) from scratch
  (`data/` is gitignored, so nothing from prior runs' 9+ independent transcriptions of
  this exact puzzle persisted). Clue text from `data/images/2026-05-28.jpg`: all 15
  across + 12 down clues read and, this time, the SEPARATE narrow across-clue column
  next to the small previous-week solution grid (queue item 8's own documented gap —
  across 1/7/8/9/10/11/13 print there, not in the main clue-text column) was found and
  read in full, closing that gap for this puzzle rather than leaving those clues
  untranscribed as several earlier runs did. Validated every printed enum against the
  GRID-DERIVED slot length (`solver/grid_tools.py validate` against the already-
  committed `data/grids/2026-05-29.json`) BEFORE touching any gold answer: **0/27
  enum-sum mismatches** across every clue this run could locate printed text for, with
  exactly one disclosed, structural gap — `22 down` has a real grid slot (length 5)
  but no printed clue text could be found anywhere on the page (the text sitting where
  a "22" clue would be expected to continue reads as an unrelated feature with a
  different, non-matching enum and a contributor name appearing nowhere else in this
  puzzle — concluded to belong to the page's OTHER puzzle, "עושה שכל," not תשבץ היגיון,
  and excluded rather than guessed).

  GOLD LETTERS came from the small solved-grid recap in the FOLLOWING week's image
  (`data/images/2026-06-04.jpg`, captioned "פתרון תשבץ ההיגיון מהשבוע שעבר"),
  grid-calibrated programmatically (darkness-threshold gridline detection on both axes,
  not eyeballed): the resulting 15x11 black-cell pattern matched the committed
  `data/grids/2026-05-29.json` EXACTLY, 0/15 row mismatches. **A real, disclosed
  transcription pitfall found and fixed mid-run, not silently corrected**: reading each
  row as a small multi-cell strip (rather than one cell at a time) produced letters in
  the WRONG horizontal order for two of the fifteen rows (the top and bottom rows,
  physically adjacent to the page's header text and the clue column respectively) —
  caught not by the black-pattern check (which cannot distinguish direction on a
  near-symmetric grid and passed identically under more than one wrong hypothesis) but
  by cross-checking specific derived answers against this project's own independently-
  documented historical record for this exact puzzle (`ברישניקוב`, `יחפניות`,
  `משהרבנו`, `אנזימימ`, `הלו`, `שלג`, `ערב`, `צרנוביל`, `ישפרחימ` all named in prior
  PR bodies/log entries): several came out as real words in the WRONG letter order,
  which is what a horizontal mis-read looks like, not what an unrelated transcription
  error looks like. Re-read every affected cell individually (single-cell crops, not
  multi-letter strips) until the full, corrected grid reproduced all nine of those
  historically-named answers exactly, including one (`יחפניות`, 2 down, from fodder
  `פחות יין`) that this run's own mechanical `recall@N` pass independently reproduced
  as its one hit — a cross-check from a completely different angle than the grid
  calibration. **Two clues (1 across, 26 across) were EXCLUDED rather than forced**:
  both draw on the same two edge rows the mis-read affected, and after the same
  individual-cell re-check most (but only some spot-checked, not all eleven) cells
  still would not assemble into a letter order matching the well-known, clue-text-
  corroborated real answer for 26 across (`פחות אבל כואב`, the Geffen/Poliker song
  clue 26 itself names) or any confirmable answer for 1 across — disclosed as an
  unresolved row-direction ambiguity at the grid's own top/bottom edges rather than
  guessed at. `data/answers/by_date/2026-05-29.json` therefore carries 25 (not 27)
  answers; `solver/build_dataset.py` correctly marks the 2 excluded clues
  `answer_raw: null, len_ok: false` rather than silently dropping the row.

  THE LEVER: `solver/candidates.py` gained `phrase_split(w, words, min_part=2,
  max_parts=3)` — if `w` is not itself a `lex()` member but IS the unbroken
  concatenation of 2 (or 3) words that already ARE, return that split. Wired into
  `lexicon_coverage_eval` as a `--phrase` diagnostic (mirroring `--prefix`'s existing
  shape) AND into `hidden_candidates`/`reversal_candidates` as a real acceptance path
  (`use_phrase=True` default, `--no-phrase` ablation, threaded through `generate()`/
  `recall_eval()`/the CLI exactly like every other toggle here).

  MEASURED (executed, not estimated). `lexicon_coverage_eval` on the 25 answered
  clues: baseline coverage **36.0% (9/25)**. With `--phrase`: of the **16** gold
  answers NOT in `lex()`, **10 (62.5%) recover** via a 2-3-way split into words
  already present — `משהרבנו`→`משה`+`רבנו`; `ישפרחימ`→`יש`+`פרחימ` (PLAYBOOK/prove.py's
  own worked example); `ברישניקוב`→`בר`+`יש`+`ניקוב`; `בתזוגתו`→`בת`+`זוג`+`תו`;
  `דבריאמת`→`דבר`+`יאמת`; `קרתנימ`→`קר`+`תנימ`; `ליסט`→`לי`+`סט`; `צרנוביל`→
  `צר`+`נוביל`; `רומסרט`→`רומ`+`סרט`; `ברבית`→`בר`+`בית`. This is the LARGEST
  structural recovery this queue item has measured on any single puzzle — over 6x
  `--prefix`'s own 1-genuine-hit-of-3-raw finding on this identical puzzle
  (2026-09-16). `recall@N` with the real generator wiring: **UNCHANGED, 4.0% (1/25)**
  with `use_phrase` on or off, both mechanical-only (12.8→18.6 avg candidates,
  by_mechanism unchanged at `{'anagram': 1}`) and full-defaults (20.9→22.6 avg
  candidates). The one hit is the same `יחפניות` anagram this puzzle's history has
  reproduced independently for the sixth-plus time. Root-caused, not just observed:
  none of the 10 phrase-recoverable answers sit as a literal (or reversed) contiguous
  run of clue letters — they are charade/definition/culture-reference outputs (a
  Nobel-prize-winning dancer's surname, a biblical epithet, a movie title), so
  `hidden`/`reversal`'s narrow "is this exact clue substring (or its reverse) now
  acceptable" acceptance path structurally cannot reach them, even though the
  diagnostic proves the underlying lexicon-membership gap it targets is real and large.

  AUDIT (mandatory gate). Forbidden reads: none — only the two sanctioned puzzle
  images were read this run (`data/images/2026-05-28.jpg`, `data/images/2026-06-04.jpg`);
  14across was attempted only via bootstrap's own sanctioned step, which never
  produced `data/answers/answers_parsed.json` (confirmed absent) before being killed,
  so no gold data of any kind came from it; `data/answers/private_defs/` was not
  crawled or touched this run (confirmed absent). `lexicon.held_out_answers()` and
  `retrieve_defs.held_out()` both confirmed (computed, not assumed) to block all 25 of
  this puzzle's own gold answers — `gold_norm - blocked` empty for both. Leak-shape
  check on `phrase_split()` specifically: confirmed directly that every gold answer's
  OWN two-or-three-way split parts (e.g. `משה`/`רבנו`, `יש`/`פרחימ`, `בר`/`יש`/`ניקוב`)
  are legitimate pre-existing `lex()` members while the WHOLE gold string is not —
  the mechanism only ever recombines dictionary words that were already reachable
  before this run touched any puzzle-specific data, not a rediscovery of the gold
  answer itself. Implausibility check: `recall@N` moved 0.0 points (nothing to
  explain); `lexicon_coverage_eval --phrase`'s 62.5%-of-misses recovery is a large
  number but is DIAGNOSTIC and mechanically deterministic (a pure string-membership
  computation, no room for researcher judgment to bias it), independently spot-checked
  by hand for all 10 hits above. All 6 affected selftests (`candidates.py`,
  `lexicon.py`, `retrieve_defs.py`, `prove.py`, `substitutions.py`, `deffit.py`)
  re-run clean, including 5 new `phrase_split()`/phrase-wiring checks added to
  `candidates.py selftest` on synthetic fixtures (never this puzzle's own gold data).

  HONEST READ: this is a genuine, large, well-evidenced structural finding on the
  DIAGNOSTIC side — the multi-word-phrase category queue item 10(b) named as
  possibly "the higher-value half" really is, recovering more than 6x what the
  prefix-stripping half of this same item found. But the specific, narrow generator
  wiring this run shipped (the two mechanisms — hidden/reversal — where "is this
  candidate string acceptable" is a direct, cheap membership check) does not yet
  convert that into a recall@N gain, because this puzzle's phrase-shaped answers are
  produced by OTHER devices (charade, culture-reference, definition) that would each
  need their own, more invasive redesign to become phrase-aware. This is the same
  "closes the diagnostic/generator gap separately" shape 2026-09-13's container-entity
  work found for a different mechanism — a real, disclosed partial result, not a
  reversal of the positive diagnostic finding above it.

  NOT DONE, honestly: did not wire `phrase_split` into `anagram_candidates` (would need
  a reverse-search over the fodder's permutations, not a simple membership check — a
  structurally different, larger change) or into `substitution_candidates`/
  `container_candidates`/`charade_candidates` (each has its own fragment-source shape
  that would need its own redesign); did not measure a second puzzle; did not attempt
  queue item 10(a) (the minimum-stem-length prefix experiment, still open); did not
  crawl or grow `private_defs`; did not act on the standing "DAILY.md is a leak
  vector" observation (2026-08-22, re-noted 2026-09-18, still unaddressed); did not
  merge or otherwise act on any of the other 18+ open solver PRs; did not push to
  `main`; did not run `vercel --prod` from this branch.
